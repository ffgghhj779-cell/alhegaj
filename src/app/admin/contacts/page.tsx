...
export default async function AdminContactsPage({
  ...
  let requests: Awaited<ReturnType<typeof prisma.contactRequest.findMany>> = [];
  ...
  return (
    <AdminShell
      title="طلبات التواصل"
      description="طلبات نموذج التواصل والعقارات/المشاريع — محفوظة داخل المنصة."
    >
        ...
        {filters.map((filter) => {
          ...
          return (
            <Link
              ...
              href={
                filter.key
                  ? `/admin/contacts?status=${filter.key}`
                  : "/admin/contacts"
              }
      ...
      {requests.length === 0 ? (
        <div className="surface-card p-8 text-center">
          <p className="body-copy">لا توجد طلبات تواصل في هذا التصنيف حالياً.</p>
        </div>
      ) : (
        <ul className="space-y-3">
          {requests.map((request) => {
            ...
            return (
              <li key={request.id}>
                <Link
                  href={`/admin/contacts/${request.id}`}
                  className="surface-card flex flex-col gap-3 p-5 transition-[border-color] hover:border-gold/40 sm:flex-row sm:items-center sm:justify-between"
                >
